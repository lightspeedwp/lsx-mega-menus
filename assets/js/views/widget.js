( function( wp, $ ) {
	 ...
	 */
	api.LSXMM.WidgetView = wp.Backbone.View.extend({
		id: null,
		id_base: '',
		title: '',
		grid: null,
		...
		render: function() {
			...
			this.$el.append( $( '<div/>' )
				...
				.append( this.title )
				.append( $( '<button />' )
						.addClass( 'not-a-button lsxmm-widget-toggle' )
				)
				.append( $( '<form/>' )
						.attr( 'id', 'lsxmm-save-widget' )
				)
			);
		...
		toggleForm: function() {
			...
			if ( ! $widget.hasClass( 'lsxmm-widget-content-visible' ) ) {

				$( '.lsxmm-widget-content-visible' ).not( $widget ).animate({ 'height': '40' }, 200, function() {
					$( this ).removeClass( 'lsxmm-widget-content-visible' );
					$( this ).width( 'auto' );
				});

				$widget.addClass( 'lsxmm-widget-content-visible' );
				...
				if ( width && 0 < width ) {
					...
					if ( widgetRightEdge >= configuratorRightEdge ) {
						$widget.addClass( 'lsxmm-widget-content-overflow' );
					}
				}
				...
				*/
				...
				$widget.animate({ 'height': '40' }, 200, function() {
					$widget.removeClass( 'lsxmm-widget-content-visible' );
					$widget.removeClass( 'lsxmm-widget-content-overflow' );
					$widget.width( 'auto' );
				});
			}
		},
		...
		removeWidget: function( event ) {
			var lsxmmSidebarWidgets, widgetPosition, inactiveWidgets, removedIdBase, widget;
			...
			lsxmmSidebarWidgets = api.value( 'sidebars_widgets[lsxmm-sidebar]' )().slice();

			widgetPosition = _.indexOf( lsxmmSidebarWidgets, this.id );

			if ( widgetPosition !== -1 ) {
				lsxmmSidebarWidgets.splice( widgetPosition, 1 );
				api.value( 'sidebars_widgets[lsxmm-sidebar]' )( _( lsxmmSidebarWidgets ).unique() );
			}
		...
		isUpdating: function( isUpdating ) {
			this.$el.find( '.grid-stack-item-content' ).toggleClass( 'lsxmm-widget-updating', isUpdating );
		},
